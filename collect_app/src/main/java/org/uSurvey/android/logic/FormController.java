...
public class FormController {
    ...
    public String getLastGroupShortText() {
        if (getLastGroup() != null) {
            return getLastGroup().getShortText();
        }
        return null;
    }
...
//        try {
//            FormDef form = mFormEntryController.getModel().getForm();
//            TreeReference reference = ((TreeReference) getLastGroup().getFormElement().getBind().getReference());
//            int refSize = reference.size();
//            String refName = reference.getName(refSize - 1);
//            List<TreeElement> bindAttributes = form.getMainInstance().getBase().getChildAt(0).getChildrenWithName(refName).get(0).getBindAttributes();
//            for (TreeElement element : bindAttributes) {
//                if (element.getName().equalsIgnoreCase("label"))
//                    return String.valueOf(element.getValue().getDisplayText());
//            }
//        } catch (Exception e) {
//        }
//        return null;
