...
import org.uSurvey.android.listeners.FormListDownloaderListener;
 ...
 * @author carlhartung
 */
public class DownloadFormListTask extends AsyncTask<Boolean, String, HashMap<String, FormDetails>> {
    ...
    @Override
    protected HashMap<String, FormDetails> doInBackground(Boolean... values) {
        boolean needInstances = values[0];
        ...
        if (needInstances) {
            //Download forms with instances
            formListUrl = Collect.getInstance().getApplicationContext().getString(R.string.default_odk_instances);
            downloadPath = settings.getString(PreferencesActivity.KEY_INSTANCES_URL, formListUrl);
        } else {
            formListUrl = Collect.getInstance().getApplicationContext().getString(R.string.default_odk_formlist);
            downloadPath = settings.getString(PreferencesActivity.KEY_FORMLIST_URL, formListUrl);
        }
        ...
        if (result.errorMessage != null) {
            if (result.responseCode == 401) {
                formList.put(DL_AUTH_REQUIRED, new FormDetails(result.errorMessage, downloadListUrl).setReasonPhrase(result.getReasonPhrase()));
            } else {
                formList.put(DL_ERROR_MSG, new FormDetails(result.errorMessage, downloadListUrl).setReasonPhrase(result.getReasonPhrase()));
            }
            return formList;
        }
        ...
        if (result.isOpenRosaResponse) {
            ...
            if (!xformsElement.getName().equals("xforms")) {
                ...
                formList.put(
                        ...
                        new FormDetails(Collect.getInstance().getString(
                                R.string.parse_openrosa_formlist_failed, error), downloadListUrl));
                return formList;
            }
            ...
            for (int i = 0; i < nElements; ++i) {
                ...
                for (int j = 0; j < fieldCount; ++j) {
                    ...
                    } else if (tag.equals("instanceUrl")) {
                        instanceUrl = XFormParser.getXMLText(child, true);
                        if (instanceUrl != null && instanceUrl.length() == 0) {
                            instanceUrl = null;
                        }
                    } else if (tag.equals("instanceID")) {
                    ...
                    } else if (tag.equals("instanceName")) {
                        instanceName = XFormParser.getXMLText(child, true);
                ...
                formList.put(needInstances ? instanceID : formId, new FormDetails(formName, downloadUrl, manifestUrl, formId, (version != null) ? version : majorMinorVersion, instanceID, instanceName, instanceUrl));
            }
        } else {
            ...
            for (int i = 0; i < formsCount; ++i) {
                ...
                if (tag.equalsIgnoreCase("form")) {
                    ...
                    String instanceId = child.getAttributeValue(null, "instanceId");
                    String instanceName = child.getAttributeValue(null, "instanceName");
                    String instanceUrl = child.getAttributeValue(null, "instanceUrl");
                    ...
                    if (downloadUrl == null || formName == null) {
                        ...
                        formList.put(
                                DL_ERROR_MSG,
                                new FormDetails(Collect.getInstance().getString(
                                        R.string.parse_legacy_formlist_failed, error), downloadUrl));
                        return formList;
                    }
                    formList.put(formName, new FormDetails(formName, downloadUrl, null, formId, null, instanceId, instanceName, instanceUrl));
